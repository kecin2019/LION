...
class ShapeNet15kPointClouds(Dataset):
    def __init__(
                ...
                for mid in all_mids:
                    ...
                    if self.clip_forge_enable:
                        ...
                        render_img_path = os.path.join(
                            img_path, synset_id, mid.split("/")[-1], "img_choy2016"
                        )
                        ...
                        assert os.path.exists(
                            render_img_path
                        ), f"render img path not find: {render_img_path}"
        ...
        self.train_points = self.all_points[
            :, : min(10000, self.all_points.shape[1])
        ]  # subsample 15k points to 10k points per shape
    ...
    def __getitem__(self, idx):
        ...
        if self.clip_forge_enable:
            ...
            img_list = [
                os.path.join(img_path, p) for p in img_list if "jpg" in p or "png" in p
            ]
            assert (
                len(img_list) > 0
            ), f"get empty list at {img_path}: {os.listdir(img_path)}"
            # subset 5 image
...
def get_datasets(cfg, args):
    ...
    logger.info(
        f"get_datasets: tr_sample_size={cfg.tr_max_sample_points}, "
