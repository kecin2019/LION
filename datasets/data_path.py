...
def get_path(dataname=None):
    dataset_path = {}
    dataset_path["pointflow"] = ["/workspace/dataset/ShapeNet/ShapeNetCore.v2.PC15k/"]
    dataset_path["clip_forge_image"] = ["./data/shapenet_render/"]

    if dataname is None:
        return dataset_path
    else:
        assert (
            dataname in dataset_path
        ), f"not found {dataname}, only: {list(dataset_path.keys())}"
...
def get_cache_path():
    cache_list = [
        "/workspace/data_cache_local/data_stat/",
