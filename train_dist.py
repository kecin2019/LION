...
def main(args, config):
    ...
    _, writer = utils.common_init(
        args.global_rank, config.trainer.seed, config.save_dir, comet_key
    )
    ...
    if args.global_rank == 0:
        ...
        if len(config.bash_name) > 0 and os.path.exists(
            os.path.join(config.save_dir, config.bash_name.split("/")[-1])
        ):
            writer.log_asset(
                os.path.join(config.save_dir, config.bash_name.split("/")[-1])
            )
    ...
    if os.path.exists(ckpt_dir) and os.path.exists(snapshot_file):
        ...
        logger.info(
            "not find any checkpoint: {}, (exist={}), or snapshot {}, (exist={})",
            ckpt_dir,
    ...
    if args.resume or args.eval_generation:
        ...
        logger.info(
            "Resuming training from {}; if you dont want resume training, edit the cmt to change the exp name",
            args.pretrained,
        )
...
def get_args():
    ...
    parser.add_argument(
        "--exp_root", type=str, default="exp", help="location of the results"
    )
    ...
    parser.add_argument(
        "--skip_sample", type=int, default=0, help="only eval nll, no sampling"
    )
    parser.add_argument("--skip_nll", type=int, default=0, help="skip eval nll ")
    # data
    parser.add_argument(
        "--ntest",
        type=str,
    ...
    parser.add_argument(
        "--master_address", type=str, default="192.168.0.16", help="address for master"
    )
    parser.add_argument(
        "--seed", type=int, default=1, help="seed used for initialization"
    )
    parser.add_argument(
        "--config", type=str, help="The configuration file.", default="none"
    )
    parser.add_argument(
        "opt",
    ...
    parser.add_argument("--eval_generation", default=False, action="store_true")
    parser.add_argument(
